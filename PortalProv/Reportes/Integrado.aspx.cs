...
{
    public partial class Integrado : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (_Id != null)
            {
                ...
                using (System.IO.FileStream MyFileStream = new System.IO.FileStream(Nombretemp, System.IO.FileMode.Create))
                {
                    ...
                    try { FacturaCliente(Report, oDoc, PDFwriter); } catch { }

                    // Orden de Compra Sistema
                    try { IntegrarOCS(Report, oDoc, PDFwriter); } catch { }

                    // Entrega de Compra Sistema
                    try { IntegrarEMS(Report, oDoc, PDFwriter); } catch { }
                    // Factura SAP
                    try { IntegrarFactSAP(Report, oDoc, PDFwriter); } catch { }
                    try { IntegrarFactSAPDiario(Report, oDoc, PDFwriter, ref xTransId); } catch { }


                    try { IntegrarContrasena(Report, oDoc, PDFwriter); } catch { }

                    try { IntegrarRetenciones(Report, oDoc, PDFwriter); } catch { }
